...
conditions(conditions == -1) = 2;  % 2 would be incongruent condition
...
for i = 1:num_sub % 2nd file has incomplete event file. So a mismatch arises b/w log and event file
    ...
    %deleting the messy reponses in the event file
    ...
    n_rows_log = 2*length(stim); % two times because the event files has stimuli and responses separately listed
    col1 = zeros(n_rows_log,1);
    col2 = zeros(n_rows_log,1);
    col3 = zeros(n_rows_log,1);
    %col4 = char(zeros(n_rows_log,5));  
    col4 = strings(n_rows_log,1);
    col5 = zeros(n_rows_log,1);
    ...
    %adding trial values
    col1_trial(1:3:end) = col1(1:2:end) - 10;
    col2_trial(:) = 1;
    col3_trial(1:3:end,:) = 101:356;
    col4_trial =  string(col4_trial);
    col4_trial(:) = 'Trig.';
    col4_trial =  char(col4_trial);
    col5_trial = string(col5_trial);
    col5_trial(1:3:end) = string(101:356);
