...
function y = tSOBI_STR_betterSMART(eegfilename, eegfile, evtfile, sfpfile, outputDir, chunks)
   ...
   % Creating trial of interest (toi) variable by reading data from the Event file.
    evts = textread(evtfile,'%s');  % textscan required more lines (P)
...
%     end 
    ...
    n_origtrig = round((size(evts,1) - 4)/7);
    origtrig = zeros(n_origtrig,1);
    origtmu = zeros(n_origtrig,1);
    ...
    for i = 5:7:size(evts,1)    % the gap of 7 is there because evts file has everything in one column. So, all the rows are converted to a column (P)
        origtmu(j) = str2double(cell2mat(evts(i)));    % at the end of the loop, the array are of dimension: 1x1032 (P)
        origtrig(j)= str2double(cell2mat(evts(i+2)));  % at the end of the loop, the array are of dimension: 1x1032 (P)
    ...
    tmu = origtmu(origtrig>1000); 
    trig = origtrig(origtrig>1000);
...
%     end
    ...
    toiAll = zeros(size(trig,1),2);
    trigAll = trig;
    tmu = tmu./1e6; % converting to seconds to multiply by # of samples.
    toiAll(:,1) = round(tmu*2048) - 512;%Set the stim-locked epoch range in samples            512 ~ 250 ms      (P)
    toiAll(:,2) = round(tmu*2048) + 3584; % 3584 ~ 1750 ms (P)
...
function [y, acSt] = findACFbasedEMG(St, W, toi, thresh)  % ACF means autocorrelation function (P)
    y = [];
    acSt = zeros(size(W,1),64);
    blockSize = 4096;
    for s = 1:1:size(W,1)  % size(W,1) = m
        acst = zeros(size(toi,1),64); 
...
function [y pp psStNL freq] = findPeaks(St, hdr, W, toi)
    ...
    psSt = zeros(size(W,1),(NFFT/2 + 1));
    psStNL = zeros(size(W,1),(NFFT/2 + 1));
    blockSize = 4096;
    
    % calculate power spectrum for each trial and each source.
    for s = 1:1:size(W,1)   % 88 (P)
        psst = zeros(size(toi,1),(NFFT/2 + 1));
        for t = 1:1:size(toi,1)  % n_trials = 960
           % tmp = St(s, toi(t,1):toi(t,2));
            ...
            %The length of pxx would be  (nFFT/2 + 1) (P)
        ...
        [~,l] = findpeaks(psStNL(s,:),'SORTSTR','descend','MINPEAKHEIGHT',.5); % sortstr = descend means that the largest peak will get number 1, the second largest 2, and so on (P)
...
function sT = createEmbeddedEvokedSourceTrial(hdr, W, eegfile, toi, cond,~, eegfilename,trig,~) % [hdr, W_scaled, eegfile, toiAll, 'STR',evtfile,eegfilename,trig,tmu]
    blockSize = 4096;
    ...
    sT_new = zeros(size(W,1), blockSize,no_event); % m x 4096 x 5 (P)
...
%     
    ...
    for i=1:no_event  % normalisation (P)
        ...
        sT(:,(i-1)*blockSize+1:i*blockSize)=sT_new(:,:,i);       % In the end, it would be m x 4096 x 5
    end
...
function sT = createSourceTrials(hdr, W, eegfile, toi, cond, eegfilename, trig)   % [ hdr, W_scaled, eegfile, toiAll, 'STR', eegfilename, trig] (P)
    ...
    [~, tmp2, ~] = fileparts(eegfile); % [filepath,name,ext] = fileparts(file) (P)
...
function W_scaled = scaleW(W_unscaled)
    W_scaled = zeros(size(W_unscaled,1), size(W_unscaled,2));  % can be initialized in an efficient way (P)
