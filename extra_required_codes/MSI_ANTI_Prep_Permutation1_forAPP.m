...
function MSI_ANTI_Prep_Permutation1_forAPP(xlsfile) 
...
for i=2:folder_no+1
    ...
    if flag==1             % flag means the number of trials in this dataset is greater than minimum acceptable threshold
        ...
        [EEG]=pop_loadset('APP_template_dataset.set', working_folder);
        tmp_chan=EEG.chanlocs;
        for fn=1:num_file
            if strcmp(d(fn).name,'APP_template_dataset.set')==0
...
%             end
            ...
            [EEG, indices] = pop_epoch( EEG, {code}, [-.2 0.6]);
