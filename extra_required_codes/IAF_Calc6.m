function [alpha_range,avg_psd,avg_freq,peak_freq]=IAF_Calc6(EEG)
%updated 01/27/13
mydata=EEG.data;
chn_no=EEG.nbchan;
Fs=EEG.srate;
...
% Fs=signal.fsample;
h = spectrum.welch({'Hann','periodic'}, 1000, 0);
...
% Peak Detection 
 [p,l] = findpeaks(double(avg_psd),'SORTSTR','descend','MINPEAKHEIGHT',.5);
 npeaks = 5;
 if length(l) < 5
            npeaks = length(l);
 end
...
alpha_range=[IAP*.6 IAP IAP IAP*1.2]; % change as Yukari's new request based on new findings
