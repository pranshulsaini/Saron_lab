function MSI_ANTI_EEGLAB_CONVERTER_NY_81(xlsfile,with_res_trig) 
% updtaed 12/21/2012
  ...
  for i=2:subjects_no+1
    ...
    if flag==1             % flag means the number of trials in this dataset is greater than minimum acceptable threshold
        ...
        for rep_no=1:cc-1 
              ...
              RT_sorted=sort(RT);
              label_fig=strcat(cell2mat((txt1(i,22))),'_',EEG_datasetname,'_', num2str(EEG.chanlocs(ch_no).labels));
              figure;erpimage(format_tmp,RT_sorted,[-prestim traillength+1 Fs],label_fig,10,1,'erp','cbar','vert',0);
              savefile_erpImage=strcat(working_folder,'\ERPImage_',cell2mat((txt1(i,22))),'_',cell2mat((txt1(i,1))),'_',mat2str(cell2mat(txt2(i,2))),'.pdf');
