function MSI_ANTI_Event_Correction(xlsfile,min_trail)
  clc;
  ...
  for i=2:subjects_no+1
    ...
    [status,output] = MSI_ANTI_EventFileCorr6(before_sobi_evt,after_sobi_evt);
    if status==1
   ...
   [latency,flag]=event_generator_for_ERPImage6(fixed_event,EEG_datasetname,working_folder,(cell2mat(txt2(i,4))),(cell2mat(txt2(i,5))),min_trail);   
   range = sprintf('Q%i', i);
   output={strcat(EEG_datasetname,'_RT.evt')};
   xlswrite(xlsfile,output, 1, range)
