function MSI_ANTI_EEGLAB6(xlsfile,percent_data,freq_bin_calc,vol_cond) 
% updtaed 12/21/2012
...
freq_bin=[alpha_range(1) alpha_range(4)];
  ...
  for i=2:subjects_no+1
    ...
    if flag==1             % flag means the number of trials in this dataset is greater than minimum acceptable threshold
       ...
       [status]=coherence_raster7(EEG,EEG_datasetname, working_folder, percent_data,freq_bin,vol_cond,prestim);
        if status==1
