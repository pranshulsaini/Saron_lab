...
% maximum and minimum intensities.  Changed scaling from 30% to 10% for
% increased clarity.  Increased number of possible channels to accomodate
% for virtual channel montages.
%
function y = SOBIplot_clarity(csdmul,avgmul,usr_sc_csd,usr_sc_averef)
    ...
    if isempty(usr_sc_csd)
        ...
        % making the scale 10% more than 0-300 ms time points for better
        % clarity.
        sc1 = sc3 + sc3 * 0.10;        
    else
        sc3 = usr_sc_csd;
    ...
    % change the number below (sc1)+xx to change the location of title
    % (subject's name)
    text(600,(sc1)+sc1*0.5,cell2mat(strsplit('-CSD.mul',subj)),'FontSize',14);    
...
%     mesh(dat.timeline,[1:dat.Channels],dat.data(:,:))
    mesh(dat.timeline,[1:dat.Channels],dat.data(s2,:))  %MANISH: s2 was missing in the code. Who changed this??
