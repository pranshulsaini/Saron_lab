...
%     end 
    ...
    origtrig = [];
    origtmu = [];
    origISI = [];    % I don't have this information in my event file (P)
    origBlock = [];    % I don't have this information in my event file (P)
    origRT = [];  % I don't have this information in my event file (P)
    origCount = [];  % I don't have this information in my event file (P)

    for i = 9:9:size(evts,1)    % the gap of 9 is there because evts file has everything in one column. So, all the rows are converted to a column (P)
    ...
    %         end
        elseif origtrig(i) >500 && origtrig(i)<525 % for rare trials (P)
            trig(end+1) = 4;     % This is the code for rare trials (P)
            tmu(end+1) = origtmu(i); % just copying the corresponding timestamps (P) 
    %          if origCount(i) == 1
    %         Trialstep(end+1) = 1;
    %         elseif origCount(i) == origCount(i-1)+1
    %             Trialstep(end+1) = 1;
    ...
    for i = 1:1:size(trig,2)
        toiAll(cAll,1) = round(tmu(i)*2048) - 3072; %Set the stim-locked epoch range in samples            3072 ~ 1.5 sec      (P)
        toiAll(cAll,2) = round(tmu(i)*2048) + 2458;  % 2548 ~ 1.24 sec (P)
        cAll = cAll + 1;
    end
    ...
    if chunks == 1  % No need for this condition if 'chunks' is always 1
        ...
        aST = createSourceTrials(hdr, W_scaled, eegfile, toiAll, 'CPT', eegfilename, trig); %88 x n_trial*epoch_time
...
function smart_autism(aST, eST, hdr, W, eegfile, toi, trig, cond, sfpfile)  % [aST, eST, hdr, W_scaled, eegfilename, toiAll, trigAll, 'CPT', sfpfile]
    ...
    [emgnew, ac] = findACFbasedEMG(aST, W, toi, emgThreshold);   % ac (dim: 88 X 64) contains mean autocorrelation coeff, and emgnew contains the list of found emg channels (P)
...
function [y acSt] = findACFbasedEMG(St, W, toi, thresh)  % ACF means autocorrelation function (P)
    ...
    acSt = [];
    blockSize = 5530;
    for s = 1:1:size(W,1)  % size(W,1) = 88
        acst = []; 
        ...
        for t = 1:1:size(toi,1)  % n_trials = 960
            %  ac = acf_old(St(s, toi(t,1):toi(t,2))',64);
             ...
             acst(t,:) = ac.ac; % autocorrelation coefficients. Final size of acst would be: 960 x 64   (P)
        end
    ...
    for s = 1:1:size(W,1)   % 88 (P)
        ...
        for t = 1:1:size(toi,1)  % n_trials = 960
           % tmp = St(s, toi(t,1):toi(t,2));
            ...
            %[pxx, f] = pwelch(a,hanning(NFFT,'periodic'),0, NFFT,2048 ); (P)
            %The length of ps would be  (nFFT/2 + 1) (P)
            
            psst(t,:) = ps.Data';  % I would have to write psst(t,:) = pxx'. In the end, psst will have dim: 960 x 1351; (P)
        end
        
        % average the power spectrum for all trials for each source.
        psSt(s,:) = 10*log10(mean(psst));   % In the end, psSt will have the dimension: 88 x 1351 (P)
        psStNL(s,:) = mean(psst); % In the end, psSt will have the dimension: 88 x 1351 (P)
        freq = ps.Frequencies; % I would have to write freq = f; (P)
...
function sT = createEvokedSourceTrial(hdr, W, eegfile, toi, cond, eegfilename) % hdr, W_scaled, eegfile, toiAll, 'CPT', eegfilename (P)
    blockSize = 5530;
    sT = zeros(size(W,1), blockSize);   % 88 x 5530 (P)
...
%     sT = sT * 1e3;
  ...
  %  for t = 1:1:size(W,1)
       fprintf(fid, '%d\t%d\t%d\t%s\n', 1500*1e3, 1, 1, 'Stimulus');   % 1500 is chosen because epoched data has 1500 ms pre stimulus time (P)
  %  end
    fclose(fid); 
    
    % creating evoked source trial projections
    chData = zeros(size(W,1), size(W,1)*blockSize);   % 88 x 5530 (P)
 ...
 % creating event file
    ...
    fprintf(fid,'Tmu\tCode\tTriNo\tComnt\n');
    for t = 1:1:size(W,1)  % 88 times.The loop is to merely say that this is the same for all sources. See that the last argument has t (P)
...
function sT = createEmbeddedEvokedSourceTrial(hdr, W, eegfile, toi, cond,evtfile, eegfilename,trig,tmu) % [hdr, W_scaled, eegfile, toiAll, 'CPT',evtfile,eegfilename,trig,tmu]
    blockSize = 5530;
    no_event=2;
    sT_new = zeros(size(W,1), blockSize,no_event); % 88x 5530 x 2 (P)
...
%     
    for t = 1:1:size(toi,1)
        trigger_no=trig(t)/2;  % remember that trig was modified in such a wasy that it only has 2 or 4 values (P)
        % the data should be read in nChans x Time format.
        x = readBESAsb_data(eegfile, hdr, toi(t,1)+1, toi(t,2));
        tmp = W*x; 
        % demeaning the sources from -80: 20  msec (this is shifted by 1 sample also).
        tmp = tmp - mean(tmp(:,2908:3113),2)*ones(1,size(tmp,2));
        sT_new(:,:,trigger_no) = sT_new(:,:,trigger_no) + tmp; % adding them separately for the two triggers
...
%    pData = zeros(size(W,1), size(W,1)*blockSize,event_no);
    chData = zeros(size(W,1), size(W,1)*blockSize*no_event);    % 88x (88*5530*2)
    bias_time=0;   
    for t = 1:1:size(W,1)  % for each source
        for i=0:no_event-1
            % append projection of each source into sensor space
    ...
    for t = 1:1:size(W,1)
       for i=0:no_event-1
         %   fprintf(fid2, '%d\t%d\t%d\t%s\n', ((t+i-1)*blockSize+1)*1e3+bias_time, 1, t, strcat('Trig.',num2str(t)));
            ...
            fprintf(fid, '%d\t%d\t%d\t%s\n', ((i)*(blockSize/2048)*1e6+bias_time)+1, 1, (i+1)*2, strcat('Trig.',num2str(t))); % with conditions (P)
       end 
...
function sT = createSourceTrials(hdr, W, eegfile, toi, cond, eegfilename, trig)   % [ hdr, W_scaled, eegfile, toiAll, 'CPT', eegfilename, trig] (P)
    ...
    sT = zeros(size(W,1), size(toi,1)*(toi(1,2)- toi(1,1))); % 88 x n_trial*epoch_time
    ...
    for t = 1:1:size(toi,1)
        ...
        tmp = tmp - mean(tmp(:,2908:3113),2)*ones(1,size(tmp,2)); % As the my calculation, the time points for 1 trial are 4096, according;y [3932, 4137]. I don't know how 2908 and 3113 are chosen (P)
    ...
    for s = 1:1:size(W,1)  % for each source
        ...
        writeBESAsb_data(chData_tmp, strcat(eegfilename,'_',cond,'_Source_',num2str(s),'_Projection_Into_SensorSpace_AllTrials.dat'));    % writing reconstructed data for each source. cond ='CPT' (P)
        clear chData_tmp   % I don't think it is required since it has been assigned zero in the beginning of the loop (P)
    end 
    
    % creating event file
    fid = fopen(strcat(eegfilename,'_',cond,'_SensorSpace_AllTrials.evt'),'w');  %cond ='CPT' (P)
    ...
    for t = 1:1:size(toi,1)
         %fprintf(fid, '%d\t%d\t%d\t%s\n', toi(t,1)*1e3, 1, t, strcat('Trial:',num2str(t)));
        fprintf(fid, '%d\t%d\t%d\t%s\n', ((t-1)*(blockSize/2048)*1e6+1), 1, t, strcat('Trial:',num2str(t))); % this is the same one as sensor trial event file (P)
    end
    fclose(fid);
end
...
function [W, B, D] = jointDiagonalization(Rx, sinThreshold)
    ...
    B = preWhitenData(Rx(:,:,1)); % m x m (P)
    
    % whitening the correlation matrices
    Rx_white = zeros(size(Rx,1),size(Rx,2),size(Rx,3)-1);  % leaving the instantaneuous correlation.   Why??? (P)
