...
function reconEEGdata_forCPTdata(eegfile, reconType, dataloc, outfile) % ['CPT03811_forSOBI',1 or 2,'\\Dss02544-cmb-d\h\CPT_SOBI_Output\CPT03811_forSOBI\CPT03811_forSOBI_Sources_CPT\','CPT03811_forSOBI.out'  ]  (P)
        ...
        wfile = strrep([dataloc,outfile],'.out','_CPT_W.mat'); % Name without location: CPT03811_forSOBI_CPT_W.mat. This file was saved during the function smart_autism() in the file tSOBI_CPT_betterSMART.m (P)
        wf = load(wfile);
        ...
        toi = wf.toi;  % It contains starting and ending time of each epoch/trial w.r.t. trigger time stamp. Assuming 960 trials, the dim should be something like: 960 x 2 (P)
        trig = wf.trig; % It contains triggers (2/4). Should be around 960
 %       BlockData = wf.BlockData;
 
        pathtooutfile = [dataloc, outfile]; %'\\Dss02544-cmb-d\h\CPT_SOBI_Output\CPT03811_forSOBI\CPT03811_forSOBI_Sources_CPT\CPT03811_forSOBI.out'
...
function artifacts = findArtifacts(outfile)   % will return the channels assigned as arifacts, either emg, peaks, eog, or Reog
    ...
    if isfield(data,'textdata')  % checks if the structure has 'textdata' as one of the fields. I don't think outfile (CPT00112_forSOBI_voteforrecon.out) has any such field. So, it's mostly going to return 0 (P)
        ...
        for i = 1:1:size(data,1)  % across 88 channels (P)
            ln = strsplit(data{i}, ' ');  % It was strsplit(' ', data{i}). Does not work because the syntax in Matlab 2018 is reverse (P)
            if strcmp(ln(2), 'emg') == 1 || strcmp(ln(2), 'peaks') == 1 || strcmp(ln(2), 'eog') == 1 || strcmp(ln(2), 'Reog') == 1
...
function writeBESAevt(evtfile, toi, trig)
 ...
 %   [a,b]=find(trig ==3);
    ...
    for t = 1:1:size(toi,1)
       fprintf(fid, '%d\t%d\t%d\t%s\n', (t-1)*(blockSize/2048)*1e6+1, 1, trig(t), strcat('Trial:',num2str(t))); % Time stamp here will not be the original timestamp. It must be corrected somewhere lateron (P)
    end
    fclose(fid);  
end
