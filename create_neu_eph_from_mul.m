...
for i = 1:num_sub % 2nd file has incomplete event file. So a mismatch arises b/w log and event file
    ...
    %filename = strcat(mainpath,localpath,'recon_',sub_IDs{i},'_aux_NoBad_AvgRef_forSOBI_81chan_av-export.mul'); % the address and filename for .mul file
    filename = strcat(mainpath,localpath,'recon_',sub_IDs{i},'_aux_NoBad_AvgRef_forSOBI_neu_81chan_av-export.mul'); % the address and filename for .mul file
    ...
    for j = 1:n_iter
        ...
        filename = strcat(mainpath,localpath,sub_IDs{i},'_neu_cond_', num2str(j),'.eph');
