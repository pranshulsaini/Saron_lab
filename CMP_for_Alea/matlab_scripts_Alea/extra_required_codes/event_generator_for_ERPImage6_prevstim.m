function [latency,flag] = event_generator_for_ERPImage6_prevstim(evtfile,datasetname,newevtfile,evtcode,respcode,min_trail,prev_stim)
...
evts = textread(evtfile,'%s');
    ...
    for i = 5:1:size(evts,1)
        ...
        if mod(i,4) == 3
            ...
            if (trig==evtcode) && (prev_stim>0)
                if (str2num(cell2mat(evts(i+4)))==respcode) &&(i>9)
                    if str2num(cell2mat(evts(i-8)))==prev_stim 
                    counter=counter+1; 
