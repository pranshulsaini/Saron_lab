...
%   
  ...
  if strcmp(g.lighting,'on')
    ...
    if ~isempty(p2)
        set(p2,'DiffuseStrength',.15,'SpecularStrength',0,...  %Edited by RJH and FG 7/15/11.  Changed DiffuseStrength from .6 to .15, SpecularExponent from 5 to 500, AmbientStrength from .4 to .75
               'AmbientStrength',.75,'SpecularExponent',500)
    end;
    set(p1,'DiffuseStrength',.15,'SpecularStrength',0,...
    'AmbientStrength',.75,'SpecularExponent',500)
    lighting phong  % all this gives a matte reflectance; edited by RJH 7/14/11, changed "phong" to "none".
  end  
