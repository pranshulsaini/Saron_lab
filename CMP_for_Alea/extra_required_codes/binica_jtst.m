...
for x=1:length(flags)
  ...
  elseif strcmp(flags{x},'WeightsTempFile')
     weightstmpfile = ['binica' tmpint 'tmp.wts'];
     weightstmpfile =  [pwd '/' weightsfile];
     args{x} = weightstmpfile;
...
if weightstmpfile == 0
    fprintf(fid,'%s %s\n','WeightsTempFile',['binica' tmpint 'tmp.wts']);
end
fclose(fid);
if ~exist(scriptfile)
...
%
   ...
   envtmp = getenv('LD_LIBRARY_PATH');
   setenv('LD_LIBRARY_PATH','');
%  for now until box fixed it  system(['/home/jason/binica/ica_linux < ' scriptfile]);
    system(['/data/common/matlab/ica_linux2.4 < ' scriptfile]);
   setenv('LD_LIBRARY_PATH',envtmp);
